...
def enrich_with_lookups_sql(
    ...
    tgt_cols = _list_columns(connection, table)  # resolve por regclass

    with connection.cursor() as cur:
        for col, cfg in mapping.items():
            src_table   = cfg['source']          # ex.: 'pipelines'
            src_schema  = cfg.get('schema')      # ex.: 'public' (opcional)
            src_key     = cfg['key']
            src_val     = cfg['value_col']
            target_col  = cfg['target_col']

            # monta nome qualificado que o Postgres entende em regclass e no SQL
