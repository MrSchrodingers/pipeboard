...
from core.schemas.activity_type_schema import ActivityType # Ajustado para o nome do seu arquivo
...
def sync_pipedrive_activity_types_flow():
    logger = get_run_logger()
    flow_name_label = "ActivityTypesSync"
    ...
    try:
        activity_type_repo = RepositorioBase(
            "atividade_tipos", 
            SchemaConfig(
                pk=['id'],
                types={
        ...
        activity_type_core_columns = [
            'id', 'name', 'key_string', 'icon_key', 'active_flag', 
            'color', 'is_custom_flag', 'order_nr', 'add_time', 'update_time'
        ]

        activity_types_synchronizer = PipedriveEntitySynchronizer(
            entity_name="ActivityType",
            pydantic_model_main=ActivityType,
            repository=activity_type_repo,
            api_endpoint_main="/activityTypes",
            api_endpoint_fields=None, 
            pydantic_model_field=None, 
            core_columns=activity_type_core_columns,
            specific_field_handlers={},
            utils_module=utils
        )

        logger.info("Starting Activity Types synchronization...")
        processed_count = activity_types_synchronizer.run_sync()
        logger.info(f"Activity Types synchronization completed. Processed {processed_count} records.")
        metrics.etl_last_successful_run_timestamp.labels(flow_type=flow_name_label).set_to_current_time()

        if activity_type_repo.schema_config.allow_column_dropping:
            logger.info(f"Attempting to drop fully null columns from '{activity_type_repo.table_name}' table...")
            try:
                activity_type_repo.drop_fully_null_columns(protected=activity_type_core_columns)
                logger.info(f"Drop fully null columns for '{activity_type_repo.table_name}' completed.")
            except Exception as e_drop:
                logger.error(f"Failed to drop fully null columns for '{activity_type_repo.table_name}': {str(e_drop)}", exc_info=True)

    except Exception as e:
        metrics.etl_failure_counter.labels(flow_type=flow_name_label).inc()
        logger.error(f"Activity Types synchronization failed: {str(e)}", exc_info=True)
        raise
    finally:
